...
class TCDScheduler(SchedulerMixin, ConfigMixin):
    ...
    def step(
        ...
        print(f"prev timestep: {prev_timestep}; modified: {timestep_s}")

        sigmas = np.sqrt(1 - self.alphas_cumprod)
        print(f"sigmas_prev: {sigmas[prev_timestep]}")
        print(f"sigmas_s: {sigmas[timestep_s]}")
        ...
        if eta > 0:
            if self.step_index != self.num_inference_steps - 1:
                ...
                sample_mix = (alpha_prod_t_prev / alpha_prod_s).sqrt()
                noise_mix = (1 - alpha_prod_t_prev / alpha_prod_s).sqrt()

                prev_sample = sample_mix * pred_noised_sample + noise_mix * noise
            else:
                noise_mix = 1.0
                sample_mix = 0.0
        ...
        print(f"noise mix: {noise_mix}; sample mix: {sample_mix}; ({noise_mix + sample_mix})")
